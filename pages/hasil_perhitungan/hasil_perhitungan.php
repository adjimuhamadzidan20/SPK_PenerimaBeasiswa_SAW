...
<?php  
  ...
  $dataPref = "SELECT * FROM hasil_preferensi";
  $retPref = mysqli_query($koneksi_db, $dataPref);
  $jumlahPref = mysqli_num_rows($retPref);
?>

<?php  
  if ($jumlahNorm == 0 && $jumlahPref == 0) {
?>
  <div class="mb-4 rounded-0">
    <p>Hasil perhitungan belum tersedia..</p>
  </div>
<?php 
  } else {
?>
  ...
  <div class="d-flex justify-content-start mb-4">
  	<button type="button" class="btn btn-custom btn-square rounded-0" data-toggle="modal" data-target="#reset">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content rounded-0 border-0">
        ...
        <div class="modal-footer">
            ...
            <a class="btn btn-custom rounded-0" href="pages/hasil_perhitungan/proses_reset_perhitungan.php"><i class="fas fa-undo fa-sm"></i> Reset</a>
        </div>
    </div>
  </div>
</div>